...
#define ASSERT_ARRNEAR_MSG(xs, ys, abs_error)                                  \
  do {                                                                         \
    ...
    for (int i : std::views::iota(0, static_cast<int>(xs.size()))) {           \
      ASSERT_NEAR_MSG(                                                         \
        xs[i], ys[i], abs_error, std::format("found in {} th data", i).c_str() \
      );                                                                       \
    }                                                                          \
  } while (0)
...
UTEST(simple_p, double)
{
  mamePID::PID<double> pid(
    testcases::simple_p::kp,
    testcases::simple_p::ki,
    testcases::simple_p::kd,
    testcases::simple_p::sp,
    -100.0,
    100.0
  );
...
{
  mamePID::PID<double> pid(
    testcases::simple_i::kp,
    testcases::simple_i::ki,
    testcases::simple_i::kd,
    testcases::simple_i::sp,
    -100.0,
    100.0
  );
