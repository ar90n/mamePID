...
CXX=clang++
CXXFLAGS=-std=c++20 -Wall -Wextra -O2 -stdlib=libc++ -I src
...
$(TEST_VECTOR_DIR)/simple_p.hpp:
	@echo "Generating simple_p.hpp"
	$(JULIA) --project=$(GEN_TESTCASES_DIR) $(GEN_TESTCASES_DIR)/src/gen_testcases.jl --name simple_p --arch PID --kp 1.0 --ki 0.0 --kd 0.0 --g 1.0 --ol 0 --n 64 > $(TEST_VECTOR_DIR)/simple_p.hpp

# Generate simple_i.hpp in the test vectors directory
$(TEST_VECTOR_DIR)/simple_i.hpp:
	@echo "Generating simple_i.hpp"
	$(JULIA) --project=$(GEN_TESTCASES_DIR) $(GEN_TESTCASES_DIR)/src/gen_testcases.jl --name simple_i --arch PID --kp 0.0 --ki 1.0 --kd 0.0 --g 1.0 --ol 0 --n 64 > $(TEST_VECTOR_DIR)/simple_i.hpp

# Generate simple_d.hpp in the test vectors directory
$(TEST_VECTOR_DIR)/simple_d.hpp:
	@echo "Generating simple_d.hpp"
	$(JULIA) --project=$(GEN_TESTCASES_DIR) $(GEN_TESTCASES_DIR)/src/gen_testcases.jl --name simple_d --arch PID --kp 0.0 --ki 0.0 --kd 1.0 --g 1.0 --ol 1 --n 64 > $(TEST_VECTOR_DIR)/simple_d.hpp
